    ...
    <Box component="form" onSubmit={submit} noValidate sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
      ...
      <Box>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
          <Typography sx={{ fontSize: '0.75rem', fontWeight: 500, color: C.slate }}>Password</Typography>
          <Link component="button" type="button" onClick={() => navigate('/forgot-password')}
            sx={{ fontSize: '0.75rem', color: C.blue, fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'none', '&:hover': { textDecoration: 'underline' } }}>
            Forgot password?
          </Link>
        </Box>
      ...
      <Box sx={{ position: 'relative', textAlign: 'center', my: 0.5 }}>
        ...
        <Typography sx={{ position: 'relative', display: 'inline-block', backgroundColor: '#fff', px: 1.5, fontSize: '0.6875rem', color: C.muted }}>or</Typography>
      </Box>

      <Button fullWidth variant="outlined" size="medium"
        sx={{ height: 38, gap: 1.25, fontSize: '0.8125rem', borderColor: C.border, color: C.inkMid, '&:hover': { borderColor: C.subtle, backgroundColor: C.surface } }}
        startIcon={
